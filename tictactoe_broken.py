...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player  #removed 'column+1'
    return game
...
def switch_player(player):
    if player == 1:  #changed sign '=' to '==' sign
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        while not available: #added a colon
            ...
            available = check_space_empty(game, row ,column) #added the column argument
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game) #removed the '#' symbol to uncomment this line of code
